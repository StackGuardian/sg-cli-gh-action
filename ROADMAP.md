...
| | |
|---|---|
| ✅ **`PolicyEvalResults`** | Retrievable, via the step's own `tirith-results.json` artifact, keyed by run id. |
| ⚠️ **`TfPlan` / `TfStateCleaned`** | Written and forwarded to the report-aggregator, but unreachable through any API. See *Upstream* below. |
...
| Bump the `WORKFLOW_STEP` revision | Dashboard schema only — the API path already works |
...
- **`comment/` sub-action** for one aggregated comment across units. Its prerequisite —
  `results-file` — already exists. Earns its keep at ~20 units, not at 3.
- **Cost policies without a pre-generated breakdown.** The step generates one lazily; confirm on a
  plan with real priced resources.
