...
Everything here runs on the runner and talks to nothing. It exists so the action is usable before
anyone has a StackGuardian account: policies committed in the repository are evaluated against the
plan, and the same pull-request comment and check run are posted.

Two deliberate choices about *how* this reuses tirith, both about not owning a second copy of
anything:

  * Rendering, masking and plan discovery are **imported** from the installed tirith
    (`tirith.platform.report` / `redact` / `discover`). A second markdown renderer would drift from
    the platform path's, and the whole value of this action is that both modes report identically.
  * Evaluation **shells out** to `tirith -policy-path P -input-path I --json`, one policy at a time.
    That stdout document is tirith's frozen contract -- pinned byte-for-byte by a golden-file test --
    which makes it the most stable interface it has. Calling the engine's Python API instead would
    couple the action to internals that carry no such promise.

The subprocess uses `sys.executable -m tirith`, so the interpreter that evaluates is the same one
whose renderer was imported above. A `tirith` on PATH could be a different installation entirely.
"""
...
try:
    from tirith.platform import discover as _discover
    from tirith.platform import redact as _redact
    from tirith.platform import report as _report

    _IMPORT_ERROR = None
except Exception as _import_error:  # pragma: no cover - exercised only on a broken install
    _discover = _redact = _report = None
...
# pathological evaluator, but an action that hangs forever is worse than one that fails.
...
def tirith_modules():
    """
    The imported tirith modules, or a clear failure.

    Deliberately not a fallback: if the renderer is unavailable, the honest outcome is a red job
    saying so. A degraded second renderer would report a different verdict from the platform path
    for the same plan, which is the one thing this design refuses to allow.
    """
...
def prepare_input(input_path, plan_file, terraform_bin, input_kind, source_dir, scratch, state_path=None):
    ...
    attribute values they compared, and those messages are copied verbatim into the pull-request
    comment -- so an unmasked local run publishes plan values to GitHub. Masking also keeps a local
    verdict identical to the platform one for the same plan, because the platform path evaluates
    the masked document too.
    ...
    """
    discover, redact, _ = tirith_modules()
...
def _evaluate_one(policy_path, input_path):
    ...
    try:
        completed = subprocess.run(
            argv + ["--json"], capture_output=True, text=True, timeout=EVALUATION_TIMEOUT
        )
...
def _failure_result(document, report, on_unknown_enforcement):
...
def evaluate(policy_paths, input_path, on_unknown_enforcement=lambda _: None):
    ...
    as `engine: <reason>`: visible in the comment, distinguishable from a real violation, and never
    mistakable for a pass. The caller still fails the job on it regardless of fail-on-error --
    "could not evaluate" is a tool failure, not a policy decision.
    """
    _, _, report = tirith_modules()
    ...
    for policy_path in policy_paths:
        ...
        if error is not None:
            ...
            if final is None:
                ...
                rule = {
                    "rule_name": rule_name,
                    "result": _failure_result(document, report, on_unknown_enforcement),
